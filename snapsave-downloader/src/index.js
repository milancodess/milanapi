  ...
  return new Promise(async (resolve) => {
    try {
      if (!url.match(/(?:https?:\/\/(web\.|www\.|m\.)?(facebook|fb)\.(com|watch)\S+)?$/) && !url.match(/(https|http):\/\/www.instagram.com\/(p|reel|tv|stories)/gi)) {
        return resolve({ developer: '@Milan Bhandari', status: false, msg: `Link Url not valid` });
      }
      ...
      if (!results.length) {
        return resolve({ developer: '@Milan Bhandari', status: false, msg: `Blank data` });
      }

      return resolve({ developer: '@Milan Bhandari', status: true, data: results });
    } catch (e) {
      return resolve({ developer: '@Milan Bhandari', status: false, msg: e.message });
    }
  });
};
