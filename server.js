...
const GITHUB_REPO = 'milan2nd/OCR_IMAGES';
const GITHUB_TOKEN = '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';
...
async function combineImages(imageUrls, combinedImgPath) {
  const images = await Promise.all(imageUrls.map(url => Jimp.read(url)));
  ...
  combinedImage.composite(images[1], imageWidth, 0);
  combinedImage.composite(images[2], 0, imageHeight);
  combinedImage.composite(images[3], imageWidth, imageHeight);
...
async function uploadToGithub(filepath, filename) {
  ...
  try {
    const { data } = await octokit.repos.createOrUpdateFileContents({
      owner: 'milan2nd',
      repo: 'OCR_IMAGES',
      path: `cacheImages/${filename}`,
      message: 'Upload image',
      content: content,
      committer: {
        name: 'Milan Bhandari',
        email: 'githubmilan@gmail.com'
      }
    });
...
app.get('/test69', async (req, res) => {
  ...
  try {
    ...
    const imageUrls = imageDetails.urls;
    const combinedImgPath = path.join(__dirname, 'collage.png');

    await combineImages(imageUrls, combinedImgPath);

    const collageUrl = await uploadToGithub(combinedImgPath, 'collage.png');
    const singleImageUrls = imageDetails.results.map(result => result.url);
